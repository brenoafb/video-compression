...
def readFile (filename):
  ...
  while (video_bytes != b''):
    ...
    rec_residuals.append(scale_from_img(img_array, vmin_n, vmax_n))
    rec_motion_vectors.append(vector)

  return rec_first_frame, rec_residuals, rec_motion_vectors

def writeFile (filename, first_frame, residuals, motion_vectors):
  with open(filename, "wb") as f:
    writeFrame(f, first_frame, None, None, None, first=True)
    for i in range(len(residuals)):
      writeFrame(f, residuals[i][0], motion_vectors[i], residuals[i][1], residuals[i][2])